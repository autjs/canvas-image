...
let ___cn___compose = (function(){
    return function compose(imgs, callback) {
        ...
        function renderImages(imgs, next) {
            if(imgs.length) {
                ...
                let img = new Image(element.w, element.h);
                img.onload = function() {
...
try {
    exports.compose = ___cn___compose;
} catch (error) {
    console.log('当前环境不支持 exports 请使用： ___cn___compose(imgArr, callback)');
}
